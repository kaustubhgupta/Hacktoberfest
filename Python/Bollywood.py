import random
movies = ('LAGAAN', 'HIGHWAY', 'THOR', 'DANGAL', 'NEWTON', 'ANEK', 'APNE', 'CHHICHHORE',
          'WAR', 'GULAAL', 'DHOOM', 'MALANG', 'STAR WARS')  # movies_name_that_you_will_get_to_play
# one movie is selected by random library
selected_movie = random.sample(movies, 1)
...
while score > 0 and '*' in starred_movie:
    ...
    if letter in lap.keys():                           # checks if the letter is present in the movie name
        ...
        for letter_guess in movie_guess:
            starred_movie = starred_movie[:letter_guess] + \
                letter+starred_movie[letter_guess+1:]
